...
namespace TDBCore.BLL
{
    public class PersonDal :BaseBll, IPersonDal
    {
        ...
        private void UpdateLocation(Guid personId, string location)
        {
            using (var context = new GeneralModelContainer())
            {
                var person = context.Persons.FirstOrDefault(p => p.Person_id == personId);
            ...
            using (var context = new GeneralModelContainer())
            {
                var effectedPeople =
                    context.Persons.Where(
                        p => (p.Person_id == person2 || p.Person_id == person1) && p.UniqueRef == Guid.Empty).ToList();
                ...
                foreach (
                    var uniqRef in
                        context.Persons.Where(
                            p => (p.Person_id == person2 || p.Person_id == person1) && p.UniqueRef != Guid.Empty))
                {
                    var @ref = uniqRef;
                    foreach (var dupePerson in context.Persons.Where(p => p.UniqueRef == @ref.UniqueRef))
                    {
                ...
                int evtCount = 1;
                foreach (var effectedPerson in effectedPeople)
                {
        ...
        public ServicePerson Get(Guid personId)
        {
            using (var context = new GeneralModelContainer())
            {
                var result = context.Persons.FirstOrDefault(o => o.Person_id == personId && !o.IsDeleted);
        ...
        public IList<ServicePerson> GetByDupeRef(Guid dupeRef)
        {
            using (var context = new GeneralModelContainer())
            {
                var persons = context.Persons.Where(o => o.UniqueRef == dupeRef && o.IsDeleted == false).ToList();
        ...
        private IEnumerable<Person> GetUniqRefDuplicates(Guid personId)
        {
            using (var context = new GeneralModelContainer())
            {
                ...
                return person != null
                    ? context.Persons.Where(o => o.UniqueRef == person.UniqueRef && o.IsDeleted == false)
                    : null;
            }
        }
        ...
        public void UpdateUniqueRefs()
        {
            ...
            using (var context = new GeneralModelContainer())
            {
                foreach (var pROw in context.Persons.Where(m => m.IsDeleted && m.TotalEvents > 1))
                {
                    var peopleToKeep = new List<Guid>();

                    var dsDbTemp =
                        context.Persons.FirstOrDefault(o => o.Person_id == pROw.Person_id && !o.IsDeleted);
                    ...
                    if (dsDbTemp != null)
                    {
                        var personList =
                            context.Persons.Where(
                                o =>
                                    o.UniqueRef == dsDbTemp.Person_id && o.IsDeleted == false &&
                                    o.Person_id != pROw.Person_id)
        ...
        public void MergeDuplicateRecords(Guid personId)
        {
            ...
            using (var context = new GeneralModelContainer())
            {
                var record = context.Persons.FirstOrDefault(o => o.Person_id == personId);
        ...
        private void TidyPersonLocations()
        {
            ...
            using (var context = new GeneralModelContainer())
            {
                ...
                foreach (
                    var prow in
                        context.Persons.Where(p => p.BirthLocationId == dummyLocationdId)
                            .Where(d => d.BirthCounty != ""
                                        && d.BirthLocation != "" && d.BirthLocation.ToLower() != "unknown" &&
                                        d.BirthLocation.Contains(d.BirthCounty)))
                {
                    char[] charsToTrim = {',', '.', ' '};
            ...
            using (var context = new GeneralModelContainer())
            {
                ...
                foreach (var county in counties)
                {
                    ...
                    foreach (
                        var personRec in
                            context.Persons.Where(p => p.BirthLocationId == dummyLocationdId)
                                .Where(o => o.BirthCounty == county1))
                    {
                        var var2 =
                            _parishsBll.GetParishsByCounty2(county)
                                .FirstOrDefault(p => personRec.BirthLocation.Contains(p.ParishName));
        ...
        public void UpdateDateEstimates()
        {
            using (var context = new GeneralModelContainer())
            {
                ...
                foreach (
                    var pRow in
                        context.Persons.Where(p => p.EstBirthYearInt == 0 && p.EstDeathYearInt == 0).ToList())
                {
                    ...
                    DateTools.CalcEstDates(pRow.BirthInt, pRow.BapInt, pRow.DeathInt, out estBirthYear,
                        out estDeathYear, out isEstBirth, out isEstDeath, pRow.FatherChristianName,
                        pRow.MotherChristianName);
