...
CellEditor.prototype.edit = function (rowIndex, columnIndex, element, value) {
    ...
    var _editableGrid = this.editablegrid;
    ...
    editorInput.onkeydown = function (event) {
        ...
        _editableGrid.getRowCount();

        var nextCell = null;
        var prevCell = null;

        _editableGrid.currentCellX = rowIndex;
        _editableGrid.currentCellY = columnIndex;


        //left
        if ((event.keyCode == 9 && event.shiftKey) || event.keyCode == 37) {

            if (_editableGrid.getColumnCount() > 1) {
                nextCell = _editableGrid.getCell(rowIndex, columnIndex - 1);


             if (nextCell.className != 'boolean') {
                    this.onblur = null;
                    this.celleditor.applyEditing(this.element, this.celleditor.getEditorValue(this));
                    this.celleditor.edit(rowIndex, columnIndex - 1, nextCell, _editableGrid.getValueAt(rowIndex, columnIndex - 1));   
                }
        ...
        if (event.keyCode == 13 || event.keyCode == 9 || event.keyCode == 39) {
            ...
            var colCount = _editableGrid.getColumnCount();
        ...
        if (event.keyCode == 38) {
            ...
            if (rowIndex > 0) {
                nextCell = _editableGrid.getCell(rowIndex - 1, columnIndex);

                if (nextCell.className != 'boolean') {
                    this.onblur = null;
                    this.celleditor.applyEditing(this.element, this.celleditor.getEditorValue(this));

                    this.celleditor.edit(rowIndex - 1, columnIndex, nextCell, _editableGrid.getValueAt(rowIndex - 1, columnIndex));
                }
            }
            return false;
        }
        ...
        if (event.keyCode == 40) {
            //rowIndex
            var rowCount = _editableGrid.getRowCount() - 1;
